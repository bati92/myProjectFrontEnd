...
export async function getStaticProps() {
    try {
        const result = await axios.get("http://127.0.0.1:8000/api/ebank");
        console.log(result.data.ebanks.data);
        console.log("http://127.0.0.1:8000/api/ebank");
        return {
            props: {
                className: "home-sticky-pin sidebar-header position-relative",
                myEbanks: result?.data?.ebanks?.data,
            },
        };
    } catch (error) {
        console.log(error);

...
export async function getStaticProps() {
    try {
        const result = await axios.get(
            "http://127.0.0.1:8000/api/data-communication"
        );
        console.log(result.data.data_communications.data);
        console.log("http://127.0.0.1:8000/api/data-communication");
        return {
            props: {
                className: "home-sticky-pin sidebar-header position-relative",
                myDataCommunications: result?.data?.data_communications?.data,
            },
        };
    } catch (error) {
        console.log(error);
...
const Home = ({ myDataCommunications }) => {
    ...
    return (
        <Wrapper>
            ...
            >
                {!myDataCommunications || myDataCommunications.length === 0 ? (
